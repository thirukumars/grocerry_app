...
const ProductListPage = ({route}) => {
    ...
    const renderItem = ({ item }) => {
        const pillWidth = item?.name?.length * 12; // Adjust the multiplier as per your preference
        ...
        return (
            <TouchableOpacity onPress={() => setSelectedCity(item)}>
                ...
                >
                    ...
                    {
                        <Text style={[styles.cityText, { color: "#12352F" }]}>{item.name}</Text>
                    }
                </LinearGradient> :
            ...
            <View style={{ backgroundColor: "white", width: "100%", height: "100%" }}>
                ...
                <View style={{ width: 400, height: 80, backgroundColor: "#FFFFFF", shadowColor: '#171717', shadowOffset: { width: -2, height: 4 }, shadowOpacity: 0.2, }}>
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', }}>
                        ...
                        {/* <Text style={styles.vegtext}>  {Object.keys(selectedCity).length !== 0 ? selectedCity?.name : "Nativity"} </Text><Text style={styles.counttext}>() </Text> */}
                        {/* categoryList[selectedCategory]?.SubCategoryCount */}
                    </View>
                </View>
                <ProductList products={route?.params?.products} cart={cart} setCart={setCart} modalOpen={modalOpen} setModalOpen={setModalOpen} type='normal'
